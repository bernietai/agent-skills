...
From a RUNNING LIST of items in the current channel, manage multiple carts for different stores on Instacart. From an agent's memory, determine the preferred BRAND and STORE before adding the item to the cart. Use Instacart's search functionality to find the item and add it to the appropriate cart. If no preferred brand or store is found, use the most popular option for that item.

Search for a store using URL format: https://www.instacart.com/store/s?k={store_name}. If the store is found, select that store. 

Once a store is selected, search the item using URL format: https://www.instacart.com/store/{store}/s?k={item}. If the BRAND is know, then include brand using URL format: https://www.instacart.com/store/{store}/s?k={brand}+{item}. 

If the item is found, add the required {quantity} of the {item} to cart.
...
Once all running items are procesed, proceed to checkout using the "Go to Checkout" button for each cart for each store.  

If user is brought to a "checkout_aisle" page, click Continue to Checkout button to proceed to checkout. Do not add any additional items to the cart at this point.

Delivery address is 235 West 56th Street Apartment 33N, New York, NY 10019. If there is an active session, the delivery address should already be set. There is no need to change the delivery address. 
